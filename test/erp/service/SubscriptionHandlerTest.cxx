...
TEST_F(SubscriptionHandlerTest, PostSubscriptionStatusParameter)
{
    ...
    const auto* body0 =
        R"--({ "resourceType": "Subscription", "status": "active", "reason": "stay in touch", "criteria": "Communication?received=null&recipient=X020406082", "channel": { "type": "websocket" } })--";
    EXPECT_NO_FATAL_FAILURE(
        testEndpoint(HttpMethod::POST, endpoint, body0, jwtOeffentliche_apotheke, HttpStatus::BadRequest));

    // Test request with empty Status is not allowed.
    const auto* body1 =
        R"--({ "resourceType": "Subscription", "status": "", "reason": "stay in touch", "criteria": "Communication?received=null&recipient=X020406082", "channel": { "type": "websocket" } })--";
    EXPECT_NO_FATAL_FAILURE(
        testEndpoint(HttpMethod::POST, endpoint, body1, jwtOeffentliche_apotheke, HttpStatus::BadRequest));

    // Test with absent status is not allowed.
    const auto* body2 =
        R"--({ "resourceType": "Subscription", "reason": "stay in touch", "criteria": "Communication?received=null&recipient=X020406082", "channel": { "type": "websocket" } })--";
    EXPECT_NO_FATAL_FAILURE(
...
TEST_F(SubscriptionHandlerTest, PostSubscriptionTelematikIdMismatch)
{
    ...
    // Test request with recipient=recipient1 but the used jwt has X020406082.
...
TEST_F(SubscriptionHandlerTest, PostSubscriptionSuccessAuth)//NOLINT(readability-function-cognitive-complexity)
{
    ...
    // Test valid request.
    const auto* body =
        R"--({ "resourceType": "Subscription", "status": "requested", "reason": "stay in touch", "criteria": "Communication?received=null&recipient=X020406082", "channel": { "type": "websocket" } })--";
    std::string responseBody;
    EXPECT_NO_FATAL_FAILURE(
...
TEST_F(SubscriptionHandlerTest, TelematicPseudonymChecks)//NOLINT(readability-function-cognitive-complexity)
{
    ...
    const std::string_view endpoint = "/Subscription";
    // Test valid request.
    const std::string recipient = "X020406082";
    ...
    // Check that different encrypted recipients do not match ((X020406082) != hash(recipient1))
...
<Subscription xmlns="http://hl7.org/fhir">
    ...
    <reason value="none" />
    <criteria value="Communication?received=null&amp;recipient=X020406082" />
    <channel>
