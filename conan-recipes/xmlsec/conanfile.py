...
from conan.tools.files import apply_conandata_patches, chdir, copy, get, replace_in_file, rm, rmdir
...
class XmlSecConan(ConanFile):
    ...
    def export_sources(self):
        for it in self.conan_data.get("patches", {}).get(self.version, []):
            copy(self, it.get("patch_file") , self.recipe_folder, self.export_sources_folder)
    ...
    def generate(self):
        if is_msvc(self):
            ...
            tc.configure_args.extend([
                ...
                "--enable-docs=no",
                "--enable-mans=no",
                f"--enable-debugging={yes_no(self.settings.build_type == 'Debug')}",
            ])
