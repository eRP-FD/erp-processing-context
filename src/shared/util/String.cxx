...
{
    Expect3(dest, "dest is null", std::invalid_argument);
    Expect3(src, "src is null", std::invalid_argument);
    Expect3(destSize >= count, "dest has not enough space", std::invalid_argument);

    // Pentesters don't like memcpy, but memcpy_s isn't available with gcc or clang. Thus, use
    // std::copy. The above assertions make this as safe as memcpy_s.
    const char* const srcBegin = static_cast<const char*>(src);
    const char* const srcEnd = srcBegin + count;
