...
std::string_view metaProfileFor(const rapidjson::Value& rxDispensation, const std::string_view partName)
{
    ...
    ModelExpect(partProfileArray.Size() == 1,
...
MedicationDispenseOperationParameters::getValidationView() const
{
    ...
    if (parameterArrayValue)
    {
        ...
        const auto medicationDispenseProfile = metaProfileFor(rxDispensation, resource::parameter::part::medicationDispense);
...
model::MedicationDispenseOperationParameters::medicationDispenses() const
{
    ...
    for (auto parameterArray = parameterArrayValue->GetArray(); const auto& parameter : parameterArray)
    {
        ...
        ModelExpect(NumberAsStringParserDocument::getStringValueFromValue(rxDispensation) == resource::parameter::rxDispensation,
        ...
        result.emplace_back(std::move(medicationDispense).value(), std::move(medication).value());
    }
    return result;
}
