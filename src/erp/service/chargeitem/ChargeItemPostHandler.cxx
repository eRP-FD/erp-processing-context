...
void ChargeItemPostHandler::handleRequest(PcSessionContext& session)
{
    ...
    A_22130.start("Check existance of 'task' parameter");
    const auto prescriptionId = parseIdFromQuery(session.request, session.accessLog, "task");
    A_22130.finish();

    auto* databaseHandle = session.database();

    A_22131.start("Check existance of referenced task in status 'completed'");
    const auto [task, prescription, receipt] = databaseHandle->retrieveTaskAndPrescriptionAndReceipt(prescriptionId);
    ...
    auto chargeItem = parseAndValidateRequestBody<model::ChargeItem>(session, SchemaType::Gem_erxChargeItem);
    ...
    chargeItem.setSupportingInfoReference(model::ChargeItem::SupportingInfoType::receipt,
                                          "Bundle/" + receipt->getId().toString());
    // removal of signature can be done on retrieval;
    A_22135.finish();

    A_22137.start("Extract PKV dispense item");
    const auto containedBinary = getDispenseItemBinary(chargeItem);
...
// Der PKV-Abgabedatensatz hat QES eines HBAs des Apothekers oder eine nonQES einer SMC-B der Apotheke.
    ...
    makeResponse(session, HttpStatus::Created, &chargeItem);
