...
public:
    explicit FhirResourceViewVerifier(const FhirStructureRepositoryBackend& repo,
                                      const FhirStructureRepositoryView* repoView = nullptr);
...
private:
    ...
    void verifyBinding(const FhirElement& element, const std::string& context);

    const FhirValueSet* tryResolve(const DefinitionKey& key, FhirElement::BindingStrength bindingStrength,
                                   const std::string& context);
    ...
    void verifyCodeSystemsAreUnambiguous();

    void verifyValueSet(const FhirValueSet& valueSet);
    void verifyValueSetExcludes(const FhirValueSet& valueSet);
    void verifyValueSetIncludes(const FhirValueSet& valueSet);
    ...
    const FhirStructureRepositoryView* mView;
    bool mVerfied = true;
