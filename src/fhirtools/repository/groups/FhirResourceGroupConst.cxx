...
public:
    ...
    findVersion(const DefinitionKey& key) const override
    {
        ...
        result.first = findVersionLocal(key);
    ...
    {
        if (auto ver = mVersions.find(key.url); ver != mVersions.end())
        {
            return (! key.version || key.version.value() == ver->second) ? std::optional{ver->second} : std::nullopt;
        }
        return std::nullopt;
    }
...
std::shared_ptr<const FhirResourceGroup> fhirtools::FhirResourceGroupConst::group() const
{
    return mGroup;
}
