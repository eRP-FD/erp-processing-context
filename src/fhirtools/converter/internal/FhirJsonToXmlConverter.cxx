                                                  ...
                                                  size_t fhirElementIndex, const rapidjson::Value& jsonMember)
{
    ...
    ModelExpect(nodes->next == nullptr, "more than one node in xhtml element: " + elementName);
    ModelExpect(nodes->name == XmlStringView{memberName},
                "xhtml root element name doesn't match member name: " + elementName);
    if (nodes->ns)
    {
        ModelExpect(nodes->ns->href == xhtmlNamespaceUri,
                    "Invalid namespace on xhtml root element: " + elementName);
    }
    else
    {
        xmlNewNs(nodes.get(), xhtmlNamespaceUri.xs_str(), nullptr);
    }
    xmlAddChildList(&targetNode, nodes.release());
    return skipElement(fhirParentType, fhirElementIndex);
}
