...
{
    return fhirtools::fromXsDateTime(dateAndTime, false /*time zone NOT optional*/, false /*seconds must exist*/);
}
...
{
    return fhirtools::fromXsDateTime("1970-01-01T" + time, true, false);
}
...
{
    return fhirtools::fromFhirDateTime(dateAndTime, true /*isSearch*/);
}


Timestamp Timestamp::fromFhirDateTime(const std::string& dateAndTime)
{
    return fhirtools::fromFhirDateTime(dateAndTime, false /*isSearch*/);
}

namespace
{
bool CountColon(const std::string& time, int num)
{
    return std::count_if(time.begin(), time.end(), [&](const auto& item) {
               return item == ':';
           }) == num;
}
}

Timestamp Timestamp::fromFhirPathTimeLiteral(const std::string& time)
{
    if (CountColon(time, 0))
        return fromXsTime(time + ":00:00");
    else if (CountColon(time, 1))
        return fromXsTime(time + ":00");
    else
        return fromXsTime(time);
}

Timestamp Timestamp::fromFhirPathDateTimeLiteral(const std::string& time)
{
    if (time.find('T') != std::string::npos)
    {
        if (CountColon(time, 0))
            return fhirtools::fromXsDateTime(time + ":00:00", true, false);
        else if (CountColon(time, 1))
            return fhirtools::fromXsDateTime(time + ":00", true, false);
        else
            return fhirtools::fromXsDateTime(time, true, false);
    }
    if (time.size() == 4)
    {
        return fhirtools::fromXsDateTime(time + "-01-01T00:00:00", true, false);
    }
    else if (time.size() == 7)
    {
        return fhirtools::fromXsDateTime(time + "-01T00:00:00", true, false);
    }
    else
    {
        return fhirtools::fromXsDateTime(time + "T00:00:00", true, false);
    }
}
...
{
    std::ostringstream s;
    s << date::format("%Y-%m-%d", date::make_zoned(fhirtools::Timestamp::GermanTimezone, mDateAndTime));
    return s.str();
}
