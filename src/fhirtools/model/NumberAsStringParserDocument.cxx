...
std::string_view NumberAsStringParserDocument::valueAsRaw(const rapidjson::Value& value)
{
...
void NumberAsStringParserDocument::removeEntry(rapidjson::Value& object, const rapidjson::Pointer& pointerToEntry)
{
    pointerToEntry.Erase(object);
}

std::optional<std::string_view> NumberAsStringParserDocument::getOptionalStringValue(const rj::Value& object,
                                                                                     const rj::Pointer& key)
{
    ...
    const std::string_view& searchValue)
{
    ...
    return findMemberInArray(array, searchKey, searchValue);
}
...
rapidjson::Value* NumberAsStringParserDocument::findMemberInArray(rapidjson::Value* array,
                                                                  const rapidjson::Pointer& searchKey,
                                                                  const std::string_view& searchValue)
{
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
    return const_cast<rj::Value*>(
        findMemberInArray(static_cast<const rapidjson::Value*>(array), searchKey, searchValue));
}

const rapidjson::Value* NumberAsStringParserDocument::findMemberInArray(const rapidjson::Value* array,
                                                                        const rapidjson::Pointer& searchKey,
                                                                        const std::string_view& searchValue)
{
...
void NumberAsStringParserDocument::removeEmptyObjectsAndArrays()
{
    removeEmptyObjectsAndArrays(*this);
}

void NumberAsStringParserDocument::removeEmptyObjectsAndArrays(rapidjson::Value& value)// NOLINT(*-no-recursion)
{
    if (value.IsObject())
    {
        for (rapidjson::Value::MemberIterator itr = value.MemberBegin(); itr != value.MemberEnd();)
        {
            removeEmptyObjectsAndArrays(itr->value);
            if ((itr->value.IsObject() && itr->value.ObjectEmpty()) || (itr->value.IsArray() && itr->value.Empty()))
            {
                itr = value.EraseMember(itr);
            }
            else
            {
                ++itr;
    ...
    {
        for (rapidjson::Value::ValueIterator itr = value.Begin(); itr != value.End();)
        {
            removeEmptyObjectsAndArrays(*itr);
            if ((itr->IsObject() && itr->GetObject().ObjectEmpty()) || (itr->IsArray() && itr->Empty()))
            {
                itr = value.Erase(itr);
            }
            else
